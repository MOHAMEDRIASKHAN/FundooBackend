    ...
    public class UserRepo : IUserRepo
    {
        ...
        public Entity.UserTable RegUser(UserPostModel userDetail)
        {
            try
            {
                ...
                usertable.Password = EncryptPassword(userDetail.Password);


                fundoo.UserDetailTable.Add(usertable);
                fundoo.SaveChanges();
                return usertable;
            }
            catch (Exception ex)
        ...
        public string LoginUser(string email, string password)
        {
            try
            {
                var result = fundoo.UserDetailTable.Where(u => u.EmailID == email && u.Password == password).FirstOrDefault();
                if(result != null)
                {
        ...
        public string ForgetPassword(string email)
        {
            try
            {
                var CheckEmail = fundoo.UserDetailTable.FirstOrDefault(e => e.EmailID == email);
                if (CheckEmail != null)
                {
        ...
        public bool UpdatePassword(String email, PasswordValidation valid)
        {
            try
            {
                ...
                {
                    var user = fundoo.UserDetailTable.Where(x => x.EmailID == email).FirstOrDefault();
